...
print(f'{c} celcius = {convert_celsius_to_fahrenheit(c)} faranheit.')
...
def calculate_slope(x1, x2, y1, y2):
    slope = (y2 - y1)/(x2 - x1)
    return slope
print(f'The slope is {calculate_slope(12, 7, 16, 4)}')
...
def reverse_list(a_list):
    ...
    for i in range(len(a_list)-1, -1, -1):
        a_rev_list.append(a_list[i])
    return a_rev_list
print(reverse_list([1,2,3,4,5,6,7,8,9,0]))

# Exercise 10
def capitalize_list_items(_list):
    capitalize_list = list()
    for j in _list:
        capitalize_list.append(j.capitalize())
    return capitalize_list
print(capitalize_list_items(['september', 'october', 'november', 'december', 'january', 'february']))
...
def evens_and_odds(num):
    ...
    for j in range(num + 1):
        if j % 2 == 0:
            ...
            no_of_odds += 1
    return f'The number of odds are {no_of_odds} and The number of evens are {no_of_evens}'
...
def calculate_std(my_list):
    ...
    standard_deviation = sqrt(my_variance)
...
def is_prime(num):
    if num <= 0:
        return f'Number must be from {1} to infinity.'
...
# Exercise 2
def unique_items(_list):
    unique_list = list()
    for i in _list:
        if i not in unique_list:
            unique_list.append(i)
        else:
            return 'The items are not unique in the list.'
    return 'The items are unique in the list.'  
print(unique_items([1,2,3,1,4]))

# Exercise 3
def same_type(my_list):
    for i in range(len(my_list)):
        for j in my_list:
            if type(my_list[i]) != type(j):
                return 'The list have different data types.'
            break
    return 'The list have same data types.'
print(same_type([1,2,4,2,6]))

# Exercise 4
def valid_variable(my_variable):
    if my_variable.isidentifier() == True:
        return f"{my_variable} is a valid python variable."
    else:
        return f"{my_variable} is not a valid python variable."
print(valid_variable('khalifa'))
...
def most_spoken_languages():
    ...
    for i in _spoken:
        for j in language:
            if language.count(j) == i and j not in most_spoken:
                most_spoken.append(j)
